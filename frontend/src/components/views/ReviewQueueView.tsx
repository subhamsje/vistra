  ...
  return (
    <div className="flex-1 h-full bg-[#0b0f19] p-8 overflow-y-auto custom-scrollbar flex flex-col space-y-6 text-xs select-none">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white tracking-wide">Human Review & Governance Queue</h1>
          <p className="text-slate-400 text-xs mt-1">Human-in-the-Loop Triage for AI Floor Predictions & Encroachment Flags</p>
        </div>

        <div className="px-3 py-1.5 rounded-lg bg-rose-500/20 border border-rose-500/30 text-rose-400 font-bold">
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="py-12 text-center text-slate-400">Loading review queue...</div>
        ) : queue.length === 0 ? (
          <div className="py-16 text-center text-slate-400 bg-slate-900/40 rounded-xl border border-white/10">
            <ShieldCheck className="w-8 h-8 text-emerald-400 mx-auto mb-2" />
            <div className="text-sm font-semibold text-white">Review Queue Clean</div>
            <div className="text-xs text-slate-500 mt-1">All vertical spatial units and 3D ULPIN identities verified.</div>
          </div>
        ) : (
          queue.map((item) => (
            <div 
              key={item.entity_id}
              className="p-5 rounded-xl bg-slate-900/80 border border-white/10 shadow-xl flex flex-col space-y-4"
            >
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-mono text-cyan-300 font-semibold text-xs">{item.ulpin_3d}</span>
                  </div>
              ...
              {/* Evidence Bar */}
              <div className="p-3 rounded-lg bg-slate-950/60 border border-white/5 flex items-center justify-between text-[11px]">
                <div className="flex items-center space-x-2 text-slate-400">
                  <Radio className="w-3.5 h-3.5 text-blue-400" />
                  <span>Evidence Source: <strong className="text-slate-200">{item.source}</strong></span>
                </div>
                <div className="text-slate-500 font-mono">Flagged: {item.flagged_at}</div>
              </div>
              ...
              <div className="flex items-center space-x-3 pt-2 border-t border-white/10">
                <input
                  ...
                  className="flex-1 h-9 px-3 bg-slate-950/80 border border-white/10 rounded-lg text-slate-200 text-xs outline-none focus:border-blue-500"
                />

                <button
                  onClick={() => handleAction(item, 'CONFIRM')}
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-semibold flex items-center space-x-1.5 transition shadow"
                >
                ...
                <button
                  ...
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-lg font-semibold flex items-center space-x-1.5 transition shadow"
                >
