...
export const BottomPipelineDock: React.FC = () => {
  const { pipelineStatus, setActiveView, isPipelineDockMinimized, setIsPipelineDockMinimized } = useCadastre();
  ...
  return (
    <>
      {isPipelineDockMinimized ? (
        <div className="h-8 bg-[#0d1321]/95 backdrop-blur-2xl border-t border-white/10 px-4 flex items-center justify-between z-20 shrink-0 select-none shadow-2xl">
          <div className="flex items-center space-x-2 text-xs">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <span className="font-semibold text-white">Pipeline: {pipelineStatus.status}</span>
            <span className="text-[10px] text-slate-400">({pipelineStatus.completed_at})</span>
          </div>
          <button
            onClick={() => setIsPipelineDockMinimized(false)}
            className="flex items-center space-x-1 text-[11px] text-cyan-400 hover:text-cyan-300 font-medium px-2 py-0.5 rounded hover:bg-white/5 transition"
          >
        ...
        <div className="h-16 bg-[#0d1321]/95 backdrop-blur-2xl border-t border-white/10 px-6 flex items-center justify-between z-20 shrink-0 select-none shadow-2xl transition-all">
